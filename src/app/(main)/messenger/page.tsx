...
import {
  ...
  Phone, Video, MoreVertical, AtSign, Users, Loader2
} from 'lucide-react';
...
export default function MessengerPage() {
  ...
  // Fetch chat rooms
  const { data: chatData, loading: roomsLoading } = useApi<ChatData>('/api/chat');

  // Fetch messages for selected room
  const { data: messagesData, loading: messagesLoading, refetch: refetchMessages } = useApi<MessagesData>(
  ...
  >(selectedRoom ? `/api/chat/${selectedRoom}/messages` : '', 'POST');

  const rooms = chatData?.rooms || [];
  const roomMessages = messagesData?.messages || [];

  // Enrich room data with member and lastMessage info
  const enrichedRooms = rooms.map(room => ({
    ...
    lastMessage: roomMessages[0] ? {
      senderName: roomMessages[0].senderName,
      content: roomMessages[0].content
    } : undefined
  })) as RoomDetails[];

  const selectedRoomData = enrichedRooms.find(r => r.id === selectedRoom);
  const channels = enrichedRooms.filter(r => r.type.includes('channel'));
  const dms = enrichedRooms.filter(r => r.type === 'dm');

  const handleSendMessage = async () => {
    if (!message.trim() || !selectedRoom) return;

    try {
      const result = await sendMessage({ content: message });
      if (result) {
        setMessage('');
      ...
      addToast({
        type: 'error',
        title: '메시지 전송 실패',
        message: error instanceof Error ? error.message : '메시지를 전송하지 못했습니다.'
      });
    }
  };
      ...
      <div className="w-72 border-r border-gray-200 bg-white flex flex-col flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {roomsLoading ? (
            ...
            <div className="px-3 py-4 text-xs text-gray-400">채널이 없습니다</div>
          ) : (
            channels.map(room => (
              <button
                key={room.id}
