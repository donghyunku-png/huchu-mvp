...
import { Search, Plus, Building2, Phone, Mail, MoreVertical } from 'lucide-react';
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">전체 {mockClients.length}개 거래처</p>
        </div>
        <div className="flex gap-2">
          ...
          <button className="px-4 py-2 bg-orange-500 text-white rounded-lg text-sm font-medium hover:bg-orange-600 flex items-center gap-1.5"><Plus className="w-4 h-4" />고객 추가</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map(client => {
          ...
          return (
            <div key={client.id} className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition-shadow cursor-pointer" onClick={() => {/* setSelectedClient(client.id) */}}>
